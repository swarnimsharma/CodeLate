...
var AdminPlans = {
    ...
    OpenCandidatePlanModal: function () {
        
        System.LoadDropDown("plan_dropdown", "candidate_plan_type");
        //AdminPlans.ResetCandidatePlanForm();
        $('#candidate_plan_modal').modal('show');
    },
    ...
    GetVendorPlansParams: function () {
        ...
        var selected_countries_name = $('#vendor_plan_country_limit').select2('data').text;
        var selected_states_name = $('#vendor_plan_state_limit').select2('data').text;
        var selected_cities_name = $('#vendor_plan_city_limit').select2('data');
        ...
        var param = {
            ...
            per_technology_limit: $('#vendor_plan_technology_limit').val(),
            technology_limit: $('#vendor_plan_technology_limit').val(),
            country_listing: selected_countries,
            state_listing: selected_states,
            city_listing: selected_cities,
            country_listing_names: selected_countries_name,
            state_listing_names: selected_states_name,
            city_listing_names: selected_cities_name,
            plan_price: $('#vendor_plan_price').val(),
            plan_duration_type: $('#vendor_plan_duration').val(),
            user_role_id: $('#user_plan_role').val(),
            bold_listing: $('#vendor_plan_bold_listing').val(),
            top_ranking: $('#vendor_plan_top_ranking').val()
        };
        return param;
    },
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != null) {
                    ...
                    $('#vendor_plan_price').val(data.plan_price);
                    $('#vendor_plan_bold_listing').val(data.bold_listing);
                    $('#vendor_plan_top_ranking').val(data.top_ranking);
                    if (data.plan_duration_type > 0) {
                        $('#vendor_plan_duration').val(data.plan_duration_type);
                    }
                    else {
                        $('#vendor_plan_duration').val("");
                    }
                    ...
                    if (data.user_role_id  > 0) {
                        $('#user_plan_role').val(data.user_role_id);
                    }    
                    else {
                        $('#user_plan_role').val('');
                    }
    ...
    ApprovePlan: function (Id, userName) {
        bootbox.confirm({
            message: 'Are you sure to confirm plan of ' + userName+' ?',
            buttons: {
                confirm: {
                    label: 'Yes',
            ...
            callback: function (result) {
                if (result) {
                    $.ajax({
                        url: "../Admin/UpdatePlanByPlanID?Id=" + Id,
                        cache: false,
                        type: "GET",
                        success: function (data) {
                            bootbox.alert('Plan Approved !');
                            if (data.status == "SUCCESS") {
                                $.ajax({
                                    method: 'GET',
                                    url: '../Admin/ApprovedCandidatePlans',
                                    success: function (data) {
                                        $('#is_approved_candidate_list_div').html(data);
                                    }
                                });
                            }
                            else {
