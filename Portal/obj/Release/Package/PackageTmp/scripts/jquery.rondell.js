...
(function($) {
  ...
  var $document, $window, Rondell, closeLightplan-box, delayCall, getActiveRondell, getLightplan-box, lightplan-boxIsVisible, resizeLightplan-box, resizeTimer, updateLightplan-box, _base;
  $.rondell || ($.rondell = {
    version: '1.0.4',
    name: 'rondell',
    lightplan-box: {
      instance: void 0,
      template: $.trim('\
        <div class="rondell-lightplan-box">\
          <div class="rondell-lightplan-box-overlay">&nbsp;</div>\
          <div class="rondell-lightplan-box-content">\
            <div class="rondell-lightplan-box-inner"/>\
            <div class="rondell-lightplan-box-close">&Chi;</div>\
            <div class="rondell-lightplan-box-prev">&nbsp;</div>\
            <div class="rondell-lightplan-box-position">1</div>\
            <div class="rondell-lightplan-box-next">&nbsp;</div>\
          </div>\
        </div>')
    },
    defaults: {
  ...
  Rondell = (function() {
    ...
    function Rondell(items, options, numItems, initCallback) {
      ...
      this.showLightplan-box = __bind(this.showLightplan-box, this);
    ...
    Rondell.prototype.bindEvents = function() {
      ...
      return this.container.delegate("." + this.classes.item, "click.rondell", function(e) {
        ...
        if (rondell._focusedItem.id === item.id) {
          if (rondell.lightplan-box.enabled) {
            e.preventDefault();
            return rondell.showLightplan-box();
          }
        } else {
          e.preventDefault();
    ...
    Rondell.prototype.shiftTo = function(idx, keepOrder) {
      ...
      if (this.lightplan-box.enabled && lightplan-boxIsVisible()) {
        this.showLightplan-box();
      }
      if (this.scrollbar.enabled) {
        scrollbarIdx = idx;
    ...
    Rondell.prototype._autoShift = function() {
      ...
      if (this.isActive() && this.isFocused() && !lightplan-boxIsVisible() && !this.autoRotation.paused) {
    ...
    Rondell.prototype.keyDown = function(e) {
      ...
      switch (keyCode) {
        ...
        case 27:
          return closeLightplan-box();
      }
    };

    Rondell.prototype.showLightplan-box = function() {
      var lightplan-box, lightplan-boxContent,
        _this = this;
      lightplan-box = getLightplan-box();
      lightplan-boxContent = $('.rondell-lightplan-box-content', lightplan-box);
      if (!lightplan-boxIsVisible()) {
        lightplan-box.add(lightplan-boxContent).css('visibility', 'hidden');
      }
      return lightplan-boxContent.stop().fadeTo(100, 0, function() {
        var attr, content, icon, iconCopy, linkTarget, linkUrl, _i, _len, _ref;
        content = $('.rondell-lightplan-box-inner', lightplan-boxContent).html(_this._focusedItem.object.html());
        $('.rondell-lightplan-box-position').text("" + _this.currentLayer + " | " + _this.maxItems);
        ...
        if (icon && !icon[0].complete) {
          ...
          lightplan-boxContent.append(_this.iconCopy);
          return iconCopy.one('load', updateLightplan-box)[0].src = _this._focusedItem.referencedImage;
        } else {
          return setTimeout(updateLightplan-box, 0);
        }
      });
    };
  ...
  resizeTimer = 0;
  resizeLightplan-box = function() {
    clearTimeout(resizeTimer);
    if (lightplan-boxIsVisible()) {
      return resizeTimer = setTimeout(updateLightplan-box, 200);
    }
  };
  lightplan-boxIsVisible = function() {
    var _ref;
    return (_ref = $.rondell.lightplan-box.instance) != null ? _ref.is(':visible') : void 0;
  };
  closeLightplan-box = function() {
    if (lightplan-boxIsVisible()) {
      return getLightplan-box().stop().fadeOut(150);
    }
  };
  getLightplan-box = function() {
    var lightplan-box;
    if (!$.rondell.lightplan-box.instance) {
      lightplan-box = $.rondell.lightplan-box.instance = $($.rondell.lightplan-box.template).appendTo($('body'));
      $('.rondell-lightplan-box-overlay, .rondell-lightplan-box-close', lightplan-box).bind('click.rondell', closeLightplan-box);
      $('.rondell-lightplan-box-prev', lightplan-box).bind('click.rondell', function() {
        return getActiveRondell().shiftLeft();
      });
      $('.rondell-lightplan-box-next', lightplan-box).bind('click.rondell', function() {
        return getActiveRondell().shiftRight();
      });
      $window.bind('resize.rondell', resizeLightplan-box);
      lightplan-box.bind("mousewheel.rondell", function(e, d, dx, dy) {
        return getActiveRondell()._onMousewheel(e, d, dx, dy);
      });
    }
    return $.rondell.lightplan-box.instance;
  };
  updateLightplan-box = function() {
    var $lightplan-box, $lightplan-boxContent, activeRondell, focusedItem, image, imageDimension, imageHeight, imageWidth, maxHeight, maxWidth, newHeight, newProps, newWidth, updateCallback, winHeight, winWidth, windowPadding;
    $lightplan-box = getLightplan-box();
    $lightplan-boxContent = $('.rondell-lightplan-box-content', $lightplan-box);
    ...
    $lightplan-box.css('display', 'block');
    image = $('img:first', $lightplan-boxContent);
    if (image.length) {
      if (!focusedItem.lightplan-boxImageWidth) {
        focusedItem.lightplan-boxImageWidth = image[0].width;
        focusedItem.lightplan-boxImageHeight = image[0].height;
        image.attr('width', focusedItem.lightplan-boxImageWidth).attr('height', focusedItem.lightplan-boxImageHeight);
      }
      imageWidth = focusedItem.lightplan-boxImageWidth;
      imageHeight = focusedItem.lightplan-boxImageHeight;
    ...
    $lightplan-box.add($lightplan-boxContent).css('visibility', 'visible');
    newWidth = $lightplan-boxContent.outerWidth();
    newHeight = $lightplan-boxContent.outerHeight();
    newProps = {
      marginLeft: -newWidth / 2,
      top: Math.max((winHeight - newHeight) / 2, 20)
    };
    updateCallback = function() {
      return typeof activeRondell.onUpdateLightplan-box === "function" ? activeRondell.onUpdateLightplan-box(focusedItem) : void 0;
    };
    if ($lightplan-boxContent.css('opacity') < 1) {
      $lightplan-boxContent.css(newProps).fadeTo(200, 1, updateCallback);
    } else {
      newProps.opacity = 1;
      $lightplan-boxContent.animate(newProps, 200, updateCallback);
    }
    return $lightplan-box.stop().fadeTo(150, 1);
  };
...
(function($) {
  ...
  return $.rondell.RondellItem = (function() {
    ...
    RondellItem.prototype.init = function() {
      ...
      if (this.isLink && this.rondell.lightplan-box.displayReferencedImages) {
    ...
    RondellItem.prototype.onAnimationFinished = function() {
      ...
      if (this.focused) {
          ...
          this.object.css("width", "300");
          this.object.css("height", "300");
